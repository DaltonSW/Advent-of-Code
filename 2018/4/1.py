...
class Elf:
    ...
    def analyzeMinutesSlept(self) -> int:
        minutes = {}
        for date in self.datesWorked:
...
[1518-11-05 00:55] wakes up"""
    ...
    while i < len(notes):
        ...
        if "begins" in note:
            ...
            newDate = Date(noteMonth, int(noteDay) + 1 if noteHour == '23' else noteDay, guardNum, noteHour, noteMinute)
    ...
    for elf in elves.values():
        total = elf.calcTotalMinutesAsleep()
        elvesSleeping[total] = elf

    sleepiestElf = elvesSleeping[max(elvesSleeping)]
    print(sleepiestElf.analyzeMinutesSlept() * int(sleepiestElf.num))
