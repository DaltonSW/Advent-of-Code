...
def main():
    ...
    instructions = []
    for _ in range(25):
        checkSignal.append(0)
        checkSignal.append(1)
    ...
    while True:
        # print(registers['a'], end=' --> ')
        if runInstructions(copy.deepcopy(registers), instructions):
            print(f"Found it! {registers['a']}")
        # print()
        registers['a'] += 1


def runInstructions(registers, instructions):
    global checkSignal
    printedVals = []
    i = 0
    while i < len(instructions):
        ...
        match split[0]:
            ...
            case 'out':
                ...
                # print(val, end=' ')
                printedVals.append(val)
                if len(printedVals) == len(checkSignal):
                    return printedVals == checkSignal
                i += 1
